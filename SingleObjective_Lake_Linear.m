%% Set up
% Testing the Linear Operating Policy
...
operating_rule = 'lake_linear'; % name of operating policy function
op_name = 'Linear Operating Policy'; % for graph titles
...
wetparams = zeros(11,3); % optimized wet season policy paramaters
dryparams = zeros(11,3); % optimized dry season policy paramaters

dryparamCHECK = zeros(11,3);
wetparamCHECK = zeros(11,3);


loop = 3:4:7; % specify the reservoir being simulated (1:11 if all)
...
for k = loop    %% Calibration
    ...
    lb  = [    eps; eps ] ; % lower bounds 
    ub  = [ pi/2.1; pi/2.1] ; % upper bounds
    ...
    wetparams(k,:) = [ y(1),s_max,delta]; % wet 
    dryparams(k,:) = [ y(2),s_max,delta]; % dry
    
    % transform variables for simulation, according to wet and dry season
    Op_param_cho = lake_linear_transform(y,idx,s_max,delta) ;
    ...
    Op_param_cho = lake_linear_transform(y,idx3,s_max3,delta3) ;    
    [ s3, Qreg3, Qspill3, ~, ~ ] = reservoir_simulation( I3, e3, env_min3, s03, s_min3, s_max3, operating_rule, Op_param_cho, delta3 ) ;
    
    % ADD SPILLS TO REGULATED FLOWS
    ...
    % Simulation/Residual graph
    [nsecal,kgecal,maecal,nseval,kgeval, maeval] = plot_figures_comb_res( S3, Qsim3, Q3, k, idx3, op_name, portion)
%     figurename = append('./graphs/',figname_str{k}, '_LAKELIN_sim_res', '.png'); % figure name and path
%     saveas(gca,figurename)
    ...
    % Simulated inflow, storage, release, spill graph
    plot_sim_with_obs( s3, S3, s_max3, I3, Q3, Qreg3, Qspill3, k, portion,op_name)
%     figurename = append('./graphs/',figname_str{k}, '_LAKELIN_sim_flows_store', '.png'); % figure name and path
%     saveas(gca,figurename)
...
%     figurename = append('./graphs/',figname_str{k}, '_LAKELIN_policy', '.png'); % figure name and path
%     saveas(gca,figurename)
    ...
    % Find a dry and wet parameter row 
